...
async fn random_delay() {
    let nanos = SystemTime::now()
        ...
        .unwrap_or(0);
    sleep(Duration::from_millis(u64::from(nanos % 1000))).await;
}
...
/// Connects the app's sqlite database, creates the Todo table, and seeds a few rows.
pub async fn db() -> Result<DatabaseConnection, Box<dyn Error + Send + Sync>> {
    let db = Database::connect("sqlite:file:simple_todo?mode=memory&cache=shared").await?;
