...
- **`cookie_secure`** defaults to `true`. Browsers drop a `Secure` cookie over plain http, so cookie-based login silently never works in local development unless you opt out - set it to `false` only for deployments that are genuinely plain http end to end.
- **`ip_source`** decides where `ctx.get_ip()` reads from. It never guesses across headers:

| `ip_source`                            | Reads                                              | Use when                         |
| -------------------------------------- | -------------------------------------------------- | -------------------------------- |
| `HttpIpSource::SocketAddr` (default)   | the `x-socket-addr` header, filled from the socket | the app is reachable directly    |
...
With `SocketAddr` you can either set `x-socket-addr` yourself from the real connection, or layer `socket_addr_layer` on the axum router, which fills it from `ConnectInfo<SocketAddr>` and overwrites anything a client sent, so the safe configuration cannot be forgotten:
